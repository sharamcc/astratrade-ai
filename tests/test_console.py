...
class ConsoleTests(unittest.TestCase):
    ...
    def test_strategy_start_fills_once_and_repeated_scheduled_key_is_idempotent(self):
        ...
        self.assertEqual(self.store.connection.execute("SELECT COUNT(*) FROM strategy_risk_decisions WHERE user_id=?", (user_id,)).fetchone()[0], 1)
    ...
    def test_legacy_risk_decisions_schema_is_extended_without_data_loss(self):
        ...
        connection.execute("INSERT INTO strategy_signals VALUES ('new-signal', 'strategy-e2e', 'user-e2e', 'v1', 'BTC-USDT', 'hold', '0', 'test', NULL, 'skipped', '2026-01-01T00:00:00+00:00')")
        store._record_risk_decision("user-e2e", "new-signal", True, "signal_action", "20", None)
        row = connection.execute("SELECT user_id, allowed, reason, threshold FROM strategy_risk_decisions WHERE signal_id='new-signal'").fetchone()
