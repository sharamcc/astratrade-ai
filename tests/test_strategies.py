...
class StrategyTests(unittest.TestCase):
    ...
    def test_moving_average_cooldown_skips_signal(self):
        result = MovingAverageStrategy({"short_window": 2, "long_window": 3, "budget_usdt": "20"}).evaluate(context((Decimal("8"), Decimal("8"), Decimal("7"), Decimal("10"),),))
        cooldown = StrategyContext(**{**context((Decimal("8"), Decimal("8"), Decimal("7"), Decimal("10"))).__dict__, "cooldown_active": True})
        self.assertEqual(result[0].action, "buy")
        self.assertEqual(MovingAverageStrategy({"short_window": 2, "long_window": 3, "budget_usdt": "20"}).evaluate(cooldown)[0].reason, "strategy_cooldown_active")
    ...
    def test_portfolio_fixed_amounts_are_capped_by_budget(self):
        strategy = PortfolioDcaStrategy({"budget_usdt": "100", "allocations": [{"instrument": "BTC-USDT", "amount_usdt": "40"}, {"instrument": "ETH-USDT", "amount_usdt": "60"}]})
        result = strategy.evaluate(context(current=Decimal("60000"), allocations=({"instrument": "ETH-USDT", "price": "3000"},)))
        self.assertEqual([signal.requested_notional for signal in result], [Decimal("40.00000000"), Decimal("60.00000000")])
        with self.assertRaises(ValueError):
            PortfolioDcaStrategy({"budget_usdt": "100", "allocations": [{"instrument": "BTC-USDT", "amount_usdt": "101"}]})
