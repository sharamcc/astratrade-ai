...
- [x] 已将最近备份恢复到独立临时 SQLite 文件，并由当前版本 `ConsoleStore` 成功打开和读取；完整性为 `ok`，读取到 12 个策略实例。未替换生产测试库。
- [x] 已现场执行测试 OAuth：设置页启动授权、测试 stub 回调完成连接、设置页显示“已连接”，随后撤销连接并显示“测试连接已撤销”；服务器记录 `oauth_connected` 与 `oauth_revoked`。测试 Provider 的 `test.invalid` 授权页无法解析属于 stub 设计，回调闭环仍已完成。
- [x] OAuth 状态重放、撤销和 HTTPS 回调已有自动化测试覆盖；本次现场回调使用一次性 state，撤销后连接状态为 `revoked`。
- [ ] 未开启外部用户试用；当前仍处于内部测试阶段。

## 4. 发布结论

当前版本满足当前内部发布验收项，仍建议保持内部测试范围；正式开放外部用户前还需单独完成 Builder 正式凭据审批、生产 Provider 配置、合规审查和外部用户告知流程。
